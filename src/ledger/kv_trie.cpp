...
namespace bubi{
	...
	bool KVTrie::Init(bubi::KeyValueDb* db, std::shared_ptr<WRITE_BATCH> batch, const std::string& prefix, int depth){
		mdb_ = db;
		prefix_ = prefix;
		batch_ = batch;
		Location location;
	...
	bool KVTrie::storage_load(const Location& location, protocol::Node& info)  {
		...
		if (stat == 1){
			...
			BUBI_EXIT("database read failed. %s", mdb_->error_desc().c_str());
		}
	}
	...
	bool KVTrie::StorageGetLeaf(const Location& location, std::string& value) {
		...
		if (stat == 1){
			...
			BUBI_EXIT("storage read failed. %s", mdb_->error_desc().c_str());
		}
	}
