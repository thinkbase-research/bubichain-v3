...
#define LOG(format, ...) \
	...
	int main(int argc, char** argv) {
		if (argc < 2) {
			fprintf(stderr, "use: --exec your_process start --log your_log_file {default[/var/log/monitoragent-daemon.log]} \n");
			exit(EXIT_FAILURE);
		}
		...
		{
			str_log = "/var/log/monitoragent-daemon.log";
		}
		else {
		...
		do 
		{
			...
			int shmid;//共享内存标识符
			//创建共享内存
			shmid = shmget((key_t)4567, sizeof(int64_t), 0666 | IPC_CREAT);
			if (shmid == -1) {
				LOG("shmget failed");
				break;
			}
